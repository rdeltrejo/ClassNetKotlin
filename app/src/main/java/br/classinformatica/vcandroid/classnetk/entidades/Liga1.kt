package br.classinformatica.vcandroid.classnetk.entidades

class Liga1 {
    var id_tela: Long = 0
    var tela: String = ""
    var indice: Int = 0
    var borda: Int = 0
    var topo: Int = 0
    var esquerdo: Int = 0
    var altura: Int = 0
    var largura: Int = 0
    var efeito: Int = 0
    var delay: Int = 0
    var cor_borda_interna1: Int = 0
    var cor_borda_interna2: Int = 0
    var estilo_borda_interna: Int = 0
    var espessura_borda_interna: Int = 0
    var cor_borda_central: Int = 0
    var cor_borda_externa1: Int = 0
    var cor_borda_externa2: Int = 0
    var estilo_borda_externa: Int = 0
    var espessura_borda_externa: Int = 0
    var cor_fundo: Int = 0
    var granulacao: Int = 0
    var transparente: Int = 0
    var cor_transparente: Int = 0
    var liga_anterior: Int = 0
    var liga_primeiro: Int = 0
    var cor_linha: Int = 0
    var espessura_linha: Int = 0
    var estilo_linha: Int = 0
    var imagem: Int = 0
    var som: Int = 0
    var enunciado: Int = 0
    var liga_errado: Int = 0
    var clicar: Int = 0
    var associasom_obj: Int = 0
    var desabilita_liga2: Int = 0
    var angulo: Int = 0
}